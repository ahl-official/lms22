...
const ensureRolePlayUnlocked = async (lessonId, user) => {
...
router.get('/start/:courseId', authenticate, async (req, res, next) => {
    try {
        ...
        if (lesson_id) {
            ...
            await ensureRolePlayUnlocked(lesson?._id, req.user);
...
router.post('/score', authenticate, async (req, res, next) => {
    try {
        ...
        await ensureRolePlayUnlocked(lesson_id || test?.lesson_id, req.user);
        ...
        if (test) {
            ...
            const best = priorAttempts.reduce((max, a) => Math.max(max, Number(a.score) || 0), 0);
            if (priorAttempts.length && best >= passBar) {
                return res.status(429).json({
                    success: false,
                    message: 'Assessment already passed. Chapter is closed.',
                });
            }
        }
        ...
        const passingScore = (test?.passing_score != null ? test.passing_score : null)
