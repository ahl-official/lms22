...
const questionScoreText = (qa) => {
  if (qa.score != null) return `${Math.round(Number(qa.score) * 10) / 10}/10`;
  if (qa.earned_points != null && qa.points != null) return `${qa.earned_points}/${qa.points}`;
  return 'N/A';
};
...
const buildStats = (attempt) => {
  ...
  const scoredQuestions = qas
    .map((qa, index) => ({ qa, index, score: Number(qa.score) }))
    .filter(item => qaHasNumericScore(item.qa) && Number.isFinite(item.score));
  const numericScores = scoredQuestions.map(item => item.score);
  ...
  const strongest = scoredQuestions.length
    ? scoredQuestions.reduce((best, item) => item.score > best.score ? item : best)
    : null;
  const needsFocus = scoredQuestions.length
    ? scoredQuestions.reduce((low, item) => item.score < low.score ? item : low)
    : null;
...
const qaHasNumericScore = (qa) => qa?.score !== null && qa?.score !== undefined && qa?.score !== '';
  ...
  const rows = (attempt.qa || []).map((qa, index) => `
    <section class="qa">
      <div class="qa-head">
        ...
        <span>${escapeHtml(questionScoreText(qa))}</span>
      </div>
...
<body>
  <main>
    ...
    <section class="section">
      ...
      <p><b>Strongest question:</b> ${stats.strongest ? `Question ${stats.strongest.index + 1} (${questionScoreText(stats.strongest.qa)})` : 'N/A'}</p>
      <p><b>Needs focus:</b> ${stats.needsFocus ? `Question ${stats.needsFocus.index + 1} (${questionScoreText(stats.needsFocus.qa)})` : 'N/A'}</p>
    </section>
    <section class="section">
