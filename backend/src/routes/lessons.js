...
const attachProgress = async (lessons, traineeId) => {
    ...
    for (const attempt of attempts) {
        ...
        // Historical pass uses score vs snapshotted attempt.passing_score (no retro chapter close).
    ...
    return lessons.map((l) => {
        ...
        const maxAttempts = l.test_id?.max_attempts || 4;
        const passBar = l.test_id?.passing_score || 60;
        const barrierMet = !!attemptsForTest && Number(attemptsForTest.best_score) >= passBar;
        const attemptsRemaining = barrierMet
            ? 0
            : Math.max(0, maxAttempts - (attemptsForTest?.attempts_used || 0));
        return {
            ...l,
