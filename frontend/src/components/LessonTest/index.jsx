...
export default function LessonTestPanel({ lesson, onComplete, locked = false, rolePlayProgress, rolePlayLoading = false }) {
    ...
    const AssessmentStatus = () => priorAttempt ? (
        <div className={`mb-5 rounded-2xl border p-4 ${priorAttempt.passed ? 'border-green-200 bg-green-50' : 'border-amber-200 bg-amber-50'}`}>
            ...
            {priorAttempt.attempts_remaining > 0 && !priorAttempt.passed && (
                <p className="text-xs text-gray-500 mt-2">You can retake this assessment if you want to improve the score.</p>
            )}
            {priorAttempt.passed && (
                <p className="text-xs text-green-700 mt-2">
                    Passed (need {priorAttempt.passing_score || linkedTest?.passing_score || 80}%). Chapter closed — no more attempts.
                </p>
            )}
            {!priorAttempt.passed && priorAttempt.attempts_remaining === 0 && (
                <p className="text-xs text-red-500 mt-2">Maximum attempts reached.</p>
            )}
        </div>
    ) : null
            ...
            <div className="card text-center py-12">
                ...
                <div className="max-w-xs mx-auto mb-5 text-left">
                    ...
                    <select
                        ...
                        disabled={priorAttempt?.passed || priorAttempt?.attempts_remaining === 0}
                    >
                ...
                {(priorAttempt?.passed || priorAttempt?.attempts_remaining === 0) && (
                    <p className="text-xs text-red-500 mt-3">
                        {priorAttempt?.passed ? 'Chapter closed after passing score.' : 'Maximum attempts reached.'}
                    </p>
                )}
            </div>
        )
