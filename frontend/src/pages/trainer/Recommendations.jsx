...
export default function TestResult() {
    ...
    // ── Fetch attempt — polls every 3s while processing ──────────────────────
    ...
    // ── Auto-generate recommendations for failed voice attempts ───────────────
    // Fires once when the attempt is scored and the trainee didn't pass.
    // Runs silently in the background — no toast, no blocking.
    ...
    // Parse rubric — handles both old format (flat numbers) and new format (with strengths/improvement_areas)
    ...
    // Fallback: if rubric is a flat object of numbers (old format)
    const flatRubric =
        rubric &&
            !rubricBreakdown &&
            !rubric.strengths &&
        ...
        <div className="p-6 max-w-3xl mx-auto">
            ...
            {isProcessing ? (
                ...
                <>
                    {/* ── Score card ────────────────────────────────────────────────── */}
                    <div
                        className={`card text-center mb-6 border-2 ${passed ? 'border-green-200' : 'border-red-200'
                            }`}
                    >
                        ...
                        <div
                            className={`inline-flex items-center gap-2 px-5 py-2 rounded-full text-sm font-semibold mb-4 ${passed
                        ...
                        <div className="grid grid-cols-3 gap-4 pt-4 border-t border-gray-100">
                            <div>
                                ...
                                <span
                                    className={`badge text-xs ${isVoice ? 'badge-coral' : 'badge-blue'
                    ...
                    {isVoice && (
                        <div className="card mb-6">
                            ...
                            {/* Improvement areas — new rich format */}
                            ...
                            {/* Rubric breakdown — new format (nested in rubric_breakdown) */}
                            {rubricBreakdown && (
                                <div className="mb-5">
                            ...
                            {/* Recommendations notice — shown only for failed attempts */}
                            {!passed && (
                                <div className="flex items-start gap-3 p-4 bg-blue-50 border border-blue-100 rounded-xl">
                                    <BookOpen size={16} className="text-blue-500 flex-shrink-0 mt-0.5" />
                                    <div>
                                        <p className="text-sm font-semibold text-blue-800">
                                            Study recommendations on the way
                                        </p>
                                        <p className="text-xs text-blue-700 mt-0.5">
                                            Personalised suggestions have been sent to your trainer for
                                            review. Once approved, they'll appear in your recommendations
                                            panel.
                                        </p>
                                    </div>
                                </div>
                        ...
                        <div className="card mb-6">
                            ...
                            <div className="space-y-4">
                                {attempt.questions_snapshot.map((q, i) => (
                                    <div
                                        key={i}
                                        className={`p-4 rounded-xl border-2 ${q.is_correct
                                    ...
                                    >
                                        ...
                                        <div className="pl-6 space-y-1 text-sm">
                                            <p className={q.is_correct ? 'text-green-700' : 'text-red-700'}>
                                                Your answer:{' '}
                                                <strong>{q.user_answer || 'No answer'}</strong>
                                            </p>
                                            {!q.is_correct && (
                    ...
                    <div className="flex gap-3 flex-wrap">
                        ...
                        <button
                            onClick={() =>
                                navigate(
                                    `/trainee/courses/${attempt.course_id?._id || attempt.course_id}`
                                )
                            }
