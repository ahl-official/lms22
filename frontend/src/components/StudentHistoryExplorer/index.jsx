...
import RoundStatsPanel from '../history/RoundStatsPanel'
...
export default function StudentHistoryExplorer({ title = 'Student History', subtitle = 'Open a student, then choose Roleplaying or Assessment to see questions and answers.' }) {
  ...
  const roleplays = useMemo(
    () => withPerGroupRounds((selected?.roleplays || []).map((item) => ({
      ...item,
      lessonId: item.lesson_id || item.lessonId,
      round: item.attempt_number ?? item.round,
    }))),
    [selected]
  )
  const assessments = useMemo(
    () => withPerGroupRounds((selected?.assessments || []).map((item) => ({
      ...item,
  ...
  const stats = useMemo(
    () => buildRoundStats(mode === 'roleplaying' ? roleplays : assessments),
    [mode, roleplays, assessments]
  )
