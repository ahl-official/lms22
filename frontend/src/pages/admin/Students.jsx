...
import { Search, UserPlus, ToggleLeft, ToggleRight, Phone, Eye, EyeOff } from 'lucide-react'
    ...
    return (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-3xl shadow-2xl p-8 w-full max-w-md">
                <h2 className="text-xl font-display font-bold text-gray-800 mb-6">New Student</h2>
...
export default function AdminStudents() {
    ...
    const { data, isLoading } = useQuery({
        queryKey: ['students', search, categoryFilter],
        queryFn: () => usersAPI.getAll({ role: 'trainee', search, category_id: categoryFilter }),
    })
    ...
    const categoryMutation = useMutation({
        ...
        onSuccess: () => { qc.invalidateQueries({ queryKey: ['students'] }); toast.success('Categories updated') },
        ...
        <div className="p-6 max-w-6xl mx-auto">
            <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
                <div>
                    ...
                    <p className="text-gray-500 mt-1">{users.length} students</p>
                </div>
            ...
            <div className="flex gap-3 mb-6 flex-wrap">
                <div className="relative flex-1 min-w-48">
                    ...
                    <input className="input-field pl-10 text-sm" placeholder="Search students…" value={search} onChange={e => setSearch(e.target.value)} />
                </div>
                ...
                <div className="card overflow-hidden p-0">
                    <table className="w-full text-sm">
                        ...
                        <tbody>
                                ...
                                <tr key={user._id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                                    <td className="px-5 py-3">
                                        <div className="flex items-center gap-3">
                                            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-sage-400 to-sage-600 flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
                            ...
                            {!users.length && <tr><td colSpan={6} className="py-12 text-center text-gray-400">No students found</td></tr>}
                        </tbody>
                    </table>
                </div>
            )}

            {showModal && <CreateStudentModal onClose={() => setShowModal(false)} onCreated={() => qc.invalidateQueries({ queryKey: ['students'] })} />}
        </div>
    )
}