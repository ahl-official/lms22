...
import { rolePlayAPI, voiceTestAPI } from '../../services/api'
...
const displayText = (message) => message?.display_content || message?.content || ''

const useRolePlaySpeech = (language) => {
    ...
    const speak = useCallback(async (displayTextValue, spokenText) => {
        const textForSpeech = (spokenText || displayTextValue || '').trim()
        const textForDisplay = (displayTextValue || spokenText || '').trim()
        if (!textForDisplay) return

        stopSpeech()

        if (language === 'hi' && textForSpeech) {
            try {
                const response = await voiceTestAPI.speech({ text: textForSpeech, language: 'hi' })
        ...
        await new Promise((resolve) => {
            const utterance = new SpeechSynthesisUtterance(textForDisplay)
            utterance.rate = 0.95
            utterance.pitch = 1
...
function SummaryScreen({ summary, onRestart }) {
    ...
    const summaryText = summary.summary_display || summary.summary
    const cfg =
    ...
    return (
        <div className="space-y-4 py-1">
            <div className={`rounded-2xl p-5 text-center border ${cfg.bg} ${cfg.border}`}>
                ...
                {summaryText && (
                    <p className="text-sm text-gray-600 mt-3 max-w-xs mx-auto leading-relaxed">{summaryText}</p>
                )}
            </div>
                    ...
                    <div className="space-y-1.5">
                        {summary.strengths.map((s, i) => (
                            <div key={i} className="flex items-start gap-2 p-3 bg-green-50 rounded-xl border border-green-100">
                                ...
                                <p className="text-sm text-green-800">{summary.strengths_display?.[i] || s}</p>
                            </div>
                        ))}
                    </div>
                </div>
                    ...
                    <div className="space-y-2">
                        {summary.improvements.map((item, i) => (
                            <div key={i} className="p-3 bg-amber-50 rounded-xl border border-amber-100">
                                <p className="text-xs font-bold text-amber-800 mb-1">{item.area_display || item.area}</p>
                                <p className="text-sm text-amber-700">{item.tip_display || item.tip}</p>
                            </div>
                        ))}
                    </div>
                </div>
...
export default function RolePlayPanel({ lesson, progress, onProgressUpdate, language = 'en', onLanguageChange }) {
    const { speak, stopSpeech } = useRolePlaySpeech(language)
    ...
    const finishSession = useCallback(async (conversationOverride = conversationRef.current) => {
        ...
        try {
            ...
            try {
                ...
                const spokenSummary = nextSummary.summary || ''
                const displaySummary = nextSummary.summary_display || spokenSummary
                await speak(displaySummary, spokenSummary)
            } catch (err) {
    ...
    }, [language, lesson._id, loading, onProgressUpdate, roleplayType, scenario, selectedPersonaKey, speak])
    ...
    const startScenario = async () => {
        ...
        try {
            ...
            const res = await rolePlayAPI.startScenario({ lesson_id: lesson._id, persona: selectedPersona, language })
            const sc = res.data.scenario
            const opening = {
                role: 'character',
                content: sc.opening_line,
                display_content: sc.opening_line_display || sc.opening_line,
                coaching: null,
            }
            setScenario(sc)
            setConversation([opening])
            setPhase('ready')
            await speak(opening.display_content, opening.content)
        } catch {
    ...
    const sendAudio = async (audioBlob) => {
        ...
        try {
            ...
            const displayTranscription = res.data.display_transcription || transcription
            const coaching = res.data.coaching
            const reply = res.data.character_reply
            const replyDisplay = res.data.character_reply_display || reply
            const nextUserCount = conversationRef.current.filter(m => m.role === 'user').length + 1
            const userMessage = {
                role: 'user',
                content: transcription,
                display_content: displayTranscription,
                coaching,
                source: 'voice',
            }
            const nextConversation = nextUserCount >= MAX_ROLEPLAY_QUESTIONS
                ...
                : [...conversationRef.current, userMessage, {
                    role: 'character',
                    content: reply,
                    display_content: replyDisplay,
                    coaching: null,
                }]

            setConversation(nextConversation)
            conversationRef.current = nextConversation
            setLatestTranscript(displayTranscription)
            setLatestFeedback(coaching)
            setPhase('feedback')

            const spokenFeedback = coaching?.spoken_feedback || coaching?.tip || ''
            const displayFeedback = coaching?.spoken_feedback_display || coaching?.tip_display || spokenFeedback
            if (nextUserCount >= MAX_ROLEPLAY_QUESTIONS) {
                await speak(displayFeedback, spokenFeedback)
            } else {
                await speak(`${displayFeedback} ${replyDisplay}`, `${spokenFeedback} ${reply}`)
            }
        ...
        return (
            <div className="space-y-4">
                <div className="rounded-xl border border-gray-200 bg-gray-50 p-3">
                    <label className="text-xs font-semibold text-gray-600 block mb-1.5">Practice language</label>
                    <select
    ...
    const currentCharacterMessage = [...conversation].reverse().find(m => m.role === 'character')
    const currentPrompt = currentCharacterMessage
        ? displayText(currentCharacterMessage)
        : (scenario.opening_line_display || scenario.opening_line)
        ...
        <div className="space-y-4">
                ...
                <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                        ...
                        <button
                            onClick={() => speak(
                                currentPrompt,
                                currentCharacterMessage?.content || scenario.opening_line,
                            )}
            ...
            {latestFeedback && (
                <div className={`rounded-xl border p-3 text-sm ${feedbackTier === 'positive'
                    ...
                    <p className="text-xs leading-relaxed">{latestFeedback.tip_display || latestFeedback.what_worked_display || latestFeedback.tip || latestFeedback.what_worked}</p>
                </div>
            )}
