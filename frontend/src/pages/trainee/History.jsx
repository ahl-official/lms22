...
import RoundStatsPanel from '../../components/history/RoundStatsPanel'
import { buildRoundStats, withPerGroupRounds } from '../../utils/historyRounds'
      ...
      <div className="flex items-start justify-between gap-3 mb-3">
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            {attempt.round != null && (
              <span className="text-[11px] font-bold uppercase tracking-wide px-2 py-0.5 rounded-md bg-brand-100 text-brand-700">
                Round {attempt.round}
              </span>
            )}
...
export default function TraineeHistory() {
  ...
  const roleplays = useMemo(() => withPerGroupRounds((roleData?.data?.attempts || []).map(item => ({
    id: item._id,
    ...
    round: item.attempt_number,
    qa: roleplayPairs(item),
  }))), [roleData])

  const assessments = useMemo(() => withPerGroupRounds((attemptData?.data?.attempts || []).map(item => ({
    id: item._id,
    title: item.test_id?.title || 'Assessment',
    course_title: item.course_id?.title || 'Course',
    testId: item.test_id?._id || item.test_id,
    score: item.score,
  ...
  const assessmentStats = useMemo(() => buildRoundStats(assessments), [assessments])
  const roleplayStats = useMemo(() => buildRoundStats(roleplays), [roleplays])
  ...
  const stats = mode === 'roleplaying' ? roleplayStats : assessmentStats
  const statsLabel = mode === 'roleplaying' ? 'Roleplaying' : 'Assessment'
    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="card">
        ...
        {loading ? (
          ...
          <>
            <RoundStatsPanel stats={stats} label={statsLabel} />
            <div className="space-y-3">
              {items.map(item => (
                <AttemptCard
                  key={item.id}
