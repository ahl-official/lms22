...
router.get('/', authenticate, authorize('admin', 'trainer'), async (req, res, next) => {
  try {
    const { role, search, department_id, category_id, include_inactive } = req.query;
    const filter = {};
    // Admins can opt into inactive users; default list stays active-only.
...
router.put('/reactivate', authenticate, authorize('admin'), async (req, res, next) => {
  try {
    const q = (req.body?.search || req.body?.email || req.body?.name || '').toString().trim();
    if (!q) {
      return res.status(400).json({ success: false, message: 'search, email, or name required' });
    }
    ...
    if (!user) {
      return res.status(404).json({ success: false, message: 'No inactive user matched that search' });
    }
