...
const rolePlayLanguageInstruction = (language) => {
    if (normalizeAssessmentLanguage(language) !== 'hi') return ''
    const label = hindiStyle() === 'devanagari'
    ...
    return `
━━ LANGUAGE ━━
Write ALL character dialogue, coaching text, spoken_feedback, what_worked, and tip in ${label}.
The trainee may speak Hindi/Hinglish. Evaluate their meaning, not English grammar.
Keep JSON keys in English. Brand names and lesson-specific English terms may stay in English.`
}

const withTurnDisplayFields = async (result, language) => {
    if (normalizeAssessmentLanguage(language) !== 'hi') return result
    const [characterDisplay, tipDisplay, whatWorkedDisplay, spokenDisplay] = await toRomanHinglishDisplayMany([
        result.character_reply || '',
        result.coaching?.tip || '',
        result.coaching?.what_worked || '',
        result.coaching?.spoken_feedback || '',
    ])
    return {
        ...result,
        character_reply_display: characterDisplay || result.character_reply,
        coaching: {
            ...result.coaching,
            tip_display: tipDisplay || result.coaching?.tip,
            what_worked_display: whatWorkedDisplay || result.coaching?.what_worked,
            spoken_feedback_display: spokenDisplay || result.coaching?.spoken_feedback,
        },
    }
}

const withSummaryDisplayFields = async (summary, language) => {
    if (normalizeAssessmentLanguage(language) !== 'hi' || !summary) return summary
    const next = { ...summary }
    if (summary.summary) next.summary_display = await toRomanHinglishDisplay(summary.summary)
    if (summary.best_moment) next.best_moment_display = await toRomanHinglishDisplay(summary.best_moment)
    if (summary.recommended_focus) next.recommended_focus_display = await toRomanHinglishDisplay(summary.recommended_focus)
    if (Array.isArray(summary.strengths)) {
        next.strengths_display = await toRomanHinglishDisplayMany(summary.strengths)
    }
    if (Array.isArray(summary.improvements)) {
        next.improvements = await Promise.all(summary.improvements.map(async (item) => ({
            ...item,
            area_display: item.area ? await toRomanHinglishDisplay(item.area) : item.area,
            tip_display: item.tip ? await toRomanHinglishDisplay(item.tip) : item.tip,
        })))
    }
    return next
}
...
const formatProgressForUser = (progress, user) => {
    const formatted = formatProgress(progress)
    if (!hasDemoFullAccess(user)) return formatted
    return {
        ...formatted,
        unlocked: true,
        passed: true,
...
const formatCourseLock = (progresses, user = null) => {
    if (hasDemoFullAccess(user)) {
        return {
            locked: false,
            locked_progress: null,
            progresses: progresses.map((p) => formatProgressForUser(p, user)),
        }
    }
...
const getProgressSubjectUser = async (req, traineeId) => {
    if (traineeId?.toString() === req.user?._id?.toString()) return req.user
    return User.findById(traineeId).select('email name role roles')
}
...
router.get('/progress/:lessonId', authenticate, async (req, res, next) => {
    try {
        ...
        const subjectUser = await getProgressSubjectUser(req, traineeId)

        res.json({
            success: true,
            progress: formatProgressForUser(progress || {
                trainee_id: traineeId,
                lesson_id: lesson._id,
...
router.post('/progress', authenticate, async (req, res, next) => {
    try {
        ...
        const alreadyUnlocked = progress.passed || progress.unlocked_by_trainer || hasDemoFullAccess(req.user)
        const wasExhausted = !alreadyUnlocked && progress.attempts_used >= MAX_ATTEMPTS
        if (!hasDemoFullAccess(req.user) && !alreadyUnlocked && progress.attempts_used >= MAX_ATTEMPTS) {
            return res.status(423).json({
                success: false,
                message: 'Role playing attempts exhausted. Contact trainer to unlock test.',
                progress: formatProgressForUser(progress, req.user),
            })
        }
        ...
        res.json({ success: true, progress: formatProgressForUser(progress, req.user), attempt: formatAttempt(attempt) })
    } catch (err) { next(err) }
})
...
// PUT /api/role-play/trainee/:traineeId/grant-attempts
// Reset attempts_used counter only — does NOT delete RolePlayAttempt history.
router.put('/trainee/:traineeId/grant-attempts', authenticate, authorize('trainer', 'admin'), async (req, res, next) => {
    try {
        ...
        const before = await RolePlayProgress.find(filter)
            .select('lesson_id course_id attempts_used best_score passed unlocked_by_trainer')
            .lean()

        const result = await RolePlayProgress.updateMany(filter, {
            $set: { attempts_used: 0 },
        })

        const historyCount = await RolePlayAttempt.countDocuments({ trainee_id: traineeId })

        res.json({
            success: true,
            message: 'Attempt counter reset. Previous attempt history preserved.',
            trainee_id: traineeId,
            course_id: courseId,
            matched: result.matchedCount ?? result.n,
            modified: result.modifiedCount ?? result.nModified,
            history_attempts_preserved: historyCount,
            before: before.map((p) => ({
...
router.get('/course/:courseId/progress', authenticate, async (req, res, next) => {
    try {
        ...
        res.json({ success: true, ...formatCourseLock(progresses, subjectUser) })
    } catch (err) { next(err) }
})
...
router.post('/scenario', authenticate, async (req, res, next) => {
    try {
        const { lesson_id, scenario_type = 'objection', persona = null, language = 'en' } = req.body
        const assessmentLanguage = normalizeAssessmentLanguage(language)
        if (!lesson_id) return res.status(400).json({ success: false, message: 'lesson_id required' })
        ...
        if (hasAnyRole(req.user, ['trainee']) && !hasDemoFullAccess(req.user)) {
...
${rolePlayLanguageInstruction(assessmentLanguage)}
...
Return ONLY valid JSON, no markdown:
{
  ...
  "goal": "What the trainee must accomplish in this conversation, using the lesson's teachings."
        ...
        scenario.language = assessmentLanguage
        if (assessmentLanguage === 'hi' && scenario.opening_line) {
            scenario.opening_line_display = await toRomanHinglishDisplay(scenario.opening_line)
        }
...
// ── Shared turn logic ─────────────────────────────────────────────────────────
const processTurn = async ({ lessonTranscript, lessonTitle, scenario, conversation, userMessage, frame, roleplayNotes = '', language = 'en' }) => {
    const assessmentLanguage = normalizeAssessmentLanguage(language)
...
tier: "positive" (8-10), "constructive" (5-7), "corrective" (1-4).`
    ...
    if (isBlankResponse(userMessage)) {
        const blankReply = assessmentLanguage === 'hi'
            ? 'मुझे आपका जवाब समझ नहीं आया। कृपया एक बार फिर समझाइए।'
            : "I didn't catch your answer. Could you please explain that once more?"
        const blankCoaching = assessmentLanguage === 'hi'
            ? {
                score: 0,
                what_worked: null,
                tip: 'कोई जवाब रिकॉर्ड नहीं हुआ। आगे बढ़ने से पहले कम से कम एक lesson-specific detail बताएं।',
                spoken_feedback: 'कोई जवाब रिकॉर्ड नहीं हुआ, इसलिए यह turn zero score है।',
                tier: 'corrective',
            }
            : {
                score: 0,
        ...
        return withTurnDisplayFields({
            character_reply: blankReply,
            coaching: blankCoaching,
        }, assessmentLanguage)
    }
    ...
    return withTurnDisplayFields(parsed, assessmentLanguage)
}

// ── POST /api/role-play/turn (text input) ─────────────────────────────────────
router.post('/turn', authenticate, async (req, res, next) => {
    try {
        const { lesson_id, scenario, conversation = [], user_message, language = 'en' } = req.body
        if (!scenario || !user_message) {
            return res.status(400).json({ success: false, message: 'scenario and user_message required' })
        }

        const assessmentLanguage = normalizeAssessmentLanguage(language || scenario?.language)
        ...
        res.json({
            ...
            character_reply_display: result.character_reply_display,
...
router.post('/turn-audio', authenticate, upload.single('audio'), async (req, res, next) => {
    try {
        ...
        const { lesson_id, scenario: rawScenario, conversation: rawConversation, language = 'en' } = req.body
        ...
        try {
            ...
            return res.status(422).json({
                success: false,
                message: assessmentLanguage === 'hi'
                    ? 'Could not transcribe audio. Please speak again clearly in Hindi/Hinglish.'
                    : 'Could not transcribe audio — try again or switch to text input.',
            })
        }

        if (assessmentLanguage === 'hi' && transcription) {
            const needsRepair = isWeakHindiTranscript(transcription)
            if (needsRepair) {
                const repair = await repairHindiVoiceTranscript(transcription)
                transcription = (repair.transcript || '').trim()
                repaired = repair.repaired
                confidence = repair.confidence
            }
        }

        if (!transcription || (assessmentLanguage === 'hi' && isWeakHindiTranscript(transcription) && confidence === 'low')) {
            return res.status(422).json({
                success: false,
                message: assessmentLanguage === 'hi'
                    ? 'Speech was unclear. Please speak again clearly in Hindi/Hinglish, then submit.'
                    : 'No speech detected. Speak more clearly or switch to text input.',
            })
        }

        displayTranscription = assessmentLanguage === 'hi'
            ? await toRomanHinglishDisplay(transcription)
            : transcription
        ...
        res.json({
            ...
            coaching: result.coaching,
            transcription,
            display_transcription: displayTranscription,
            language: assessmentLanguage,
            repaired,
            confidence,
        })
...
router.post('/summary', authenticate, async (req, res, next) => {
    try {
        const { lesson_id, scenario, conversation = [], language = 'en' } = req.body
...
${rolePlayLanguageInstruction(assessmentLanguage)}`
        ...
        const summaryWithDisplay = await withSummaryDisplayFields(summary, assessmentLanguage)
        res.json({ success: true, summary: summaryWithDisplay })
    } catch (err) { next(err) }
})
